...
import { Users, User, Send, MessageSquare, Loader2 } from 'lucide-react';
...
export default function Messages() {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        // Mock data fallback if DB is empty during testing
        if (usersList.length === 0) {
          setAllUsers([
            { id: '1', displayName: 'Aisha Mohammed', course: 'Animal Health and Production' },
            { id: '2', displayName: 'David Okafor', course: 'Fisheries Technology' }
          ]);
        }
      } catch (error) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 h-[calc(100vh-4rem)] flex gap-6">
      ...
      {/* Sidebar */}
      <div className="w-80 flex-shrink-0 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col hidden md:flex overflow-hidden">
        ...
        <div className="overflow-y-auto flex-grow">
          ...
          <div className="p-3">
            ...
            <button
              onClick={() => setActiveChat({ type: 'group', id: currentUser.course || 'General' })}
          ...
          <div className="p-3">
            ...
            <div className="space-y-1">
              {allUsers.map(user => (
                <button
                  key={user.id}
                  onClick={() => setActiveChat({ type: 'private', id: user.id })}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-colors ${
